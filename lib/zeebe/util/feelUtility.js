...
export function parseVariables(variables) {
  ...
  const expressionDetailsCache = new Map();
  const scopedVariableIndex = createScopedVariableIndex(variables);

  const getCachedExpressionDetails = (expression) => {
    if (!expressionDetailsCache.has(expression)) {
      expressionDetailsCache.set(expression, getExpressionDetails(expression));
    }

    return expressionDetailsCache.get(expression);
  };
  ...
  variables.forEach(variable => {
    variable.origin.forEach(origin => {
      ...
      const expressionDetails = getCachedExpressionDetails(expression);
      ...
      const requirementAnalyses = collectRequirementAnalyses(
        expressionCandidates,
        expression,
        expressionDetails,
        getCachedExpressionDetails
      );
  ...
  for (const { variableName, targetName, origin } of localUsages) {
    const variable = findNearestScopedVariable(scopedVariableIndex, variableName, origin);
  ...
  annotateConsumedUsagesToScopedVariables(scopedVariableIndex.uniqueScopedByName, consumedVariables);

  return { resolvedVariables, consumedVariables };
}

function annotateConsumedUsagesToScopedVariables(uniqueScopedByName, consumedVariables) {
  for (const consumedVariable of consumedVariables) {
    ...
    for (const candidateName of candidateNames) {
      const scopedVariable = uniqueScopedByName.get(candidateName);
...
function findNearestScopedVariable(scopedVariableIndex, variableName, origin) {
  const scopes = [ origin, ...getParents(origin) ];

  for (const scope of scopes) {
    const variablesByName = scopedVariableIndex.byScopeId.get(scope.id);
...
function collectRequirementAnalyses(
    ...
    getExpressionDetailsForExpression = getExpressionDetails
) {
  ...
  for (const { type, value } of expressionCandidates) {
    ...
    const analysis = value === primaryExpression
      ? primaryExpressionDetails
      : getExpressionDetailsForExpression(value);
...
function buildConsumedVariables(analysisResults) {
  ...
  for (const { origin, targetName, inputs, expressionType } of analysisResults) {
    ...
    for (const inputVar of inputs) {
      ...
      const existingVariable = consumedVariables.get(key);

      if (!existingVariable) {
        consumedVariables.set(key, {
          name: inputVar.name,
          origin: undefined,
  ...
  return { consumed: Array.from(consumedVariables.values()), localUsages };
}

function createScopedVariableIndex(variables) {
  const byScopeId = new Map();
  const uniqueScopedByName = new Map();
  const scopedVariableCountByName = new Map();

  for (const variable of variables) {
    const scopeId = variable.scope && variable.scope.id;
    ...
    if (!byScopeId.has(scopeId)) {
      byScopeId.set(scopeId, new Map());
    }

    const variablesByName = byScopeId.get(scopeId);
    ...
    const scopedVariableCount = (scopedVariableCountByName.get(variableName) || 0) + 1;

    scopedVariableCountByName.set(variableName, scopedVariableCount);
    uniqueScopedByName.set(variableName, scopedVariableCount === 1 ? variable : null);
  }
