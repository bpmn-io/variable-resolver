...
export function extractConsumedVariablesFromElements(elements = []) {
  ...
  for (const { element, expression } of elementExpressions) {
    ...
    const inputMappingTargets = new Set(getInputMappingTargetNames(element));
...
function getElementExpressions(elements) {
  ...
  for (const element of elements) {
    ...
    // bpmn:SequenceFlow - condition expression
    if (is(element, 'bpmn:SequenceFlow')) {
      const conditionBody = element.conditionExpression && element.conditionExpression.body;
    ...
    // Message events - message name and subscription correlation key
    const messageRef = is(element, 'bpmn:ReceiveTask')
      ? element.messageRef
      : getEventDefinition(element, 'bpmn:MessageEventDefinition') &&
        getEventDefinition(element, 'bpmn:MessageEventDefinition').messageRef;
    ...
    if (loopCharacteristics) {
      ...
      const completionConditionBody = loopCharacteristics.completionCondition
    ...
    // Conditional event definition - condition expression
    const conditionalEventDef = getEventDefinition(element, 'bpmn:ConditionalEventDefinition');

    if (conditionalEventDef) {
      const conditionBody = conditionalEventDef.condition && conditionalEventDef.condition.body;
    ...
    // Timer event definition - timer value
    ...
    if (timerEventDef) {
      ...
      const timerBody = timerExpr && timerExpr.body;
    ...
    // zeebe:TaskSchedule - dueDate, followUpDate
    ...
    if (priorityDef && priorityDef.priority) {
      expressions.push({ element, expression: priorityDef.priority });
    }

    // zeebe:CalledElement - processId (Call Activity)
    ...
    // zeebe:CalledDecision - decisionId (Business Rule Task)
    const calledDecision = getExtensionElementsList(element, 'zeebe:CalledDecision')[0];

    if (calledDecision && calledDecision.decisionId) {
      expressions.push({ element, expression: calledDecision.decisionId });
    }

    // zeebe:AdHoc - activeElementsCollection, outputElement
    ...
    // bpmn:AdHocSubProcess - completion condition
    if (is(element, 'bpmn:AdHocSubProcess')) {
      const completionConditionBody = element.completionCondition
        && element.completionCondition.body;
