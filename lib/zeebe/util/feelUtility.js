...
import { EntriesContext, UnionContext } from './VariableContext';
...
export function parseVariables(variables) {
  ...
  variables.forEach(variable => {
    variable.origin.forEach(origin => {
      ...
      variablesToResolve.push({ variable, expression, unresolved, origin });
...
function resolveReferences(variablesToResolve, allVariables) {
  ...
  sortedVariables.forEach(({ variable, expression, unresolved, origin }) => {
    ...
    // Merge UnionContext back into a single EntriesContext for backward compat
    if (computedResult instanceof UnionContext) {
      const merged = computedResult.variants.reduce((m, variant) => {
        const val = variant instanceof EntriesContext ? variant.value : variant.value || {};
        return EntriesContext.__merge(m, val);
      }, { entries: {} });
      computedResult = new EntriesContext(merged);
    }
    ...
    // preserve both variants in a UnionContext so downstream resolution sees both origins.
    const existing = rootContext.entries[variable.name];

    if (existing) {
      const existingCtx = existing instanceof UnionContext
        ? existing.variants[existing.variants.length - 1]
        : existing;
      const existingType = getType(existingCtx.value);
      ...
      const variants = existing instanceof UnionContext
        ? [ ...existing.variants, computedResult ]
        : [ existing, computedResult ];
      rootContext.entries[variable.name] = new UnionContext(variants);
    } else {
  ...
  // Use the per-iteration origin element (not the merged multi-origin array) so that
  // subsequent _parseVariables merges create one variant per origin.
  const enrichedVariables = newVariables.map(({ newVariable, oldVariable, origin }) => {
    if (oldVariable) {
      return {
        ...newVariable,
        ...oldVariable,
        origin: origin ? [ origin ] : (oldVariable.origin ? [ ...oldVariable.origin ] : oldVariable.origin)
      };
    }
    return newVariable;
  });
...
export function toUnifiedFormat(variables) {
  ...
  for (const key in variables) {
    ...
    if (variable instanceof UnionContext) {
      variants = variable.variants.map(variant => {
        const val = variant instanceof EntriesContext ? variant.value : variant.value || {};
        return {
          ...annotate(val),
          entries: toUnifiedFormat(val.entries),
          scope: variant.scope || val.scope
        };
      });

      // Merge for backward-compat top-level fields
      variable = variable.variants.reduce((merged, variant) => {
        const val = variant instanceof EntriesContext ? variant.value : variant.value || {};
        return EntriesContext.__merge(merged, val);
      }, { entries: {} });
    } else if (variable instanceof EntriesContext) {
      const val = variable.value;
      variants = [ {
        ...annotate(val),
        entries: toUnifiedFormat(val.entries),
        scope: variable.scope || val.scope
      } ];
      variable = val;
    }
...
function filterForScope(context, variable) {
  ...
  for (const key in context.entries) {
    ...
    if (entry instanceof UnionContext) {
      const matchingVariants = entry.variants.filter(variant => {
        if (variant instanceof EntriesContext && variant.scope) {
          return validScopes.find(scope => scope.id === variant.scope.id);
        }

        // Variants without scope (e.g. from global context) pass through
        return true;
      });

      if (matchingVariants.length > 1) {
        scopedResults.entries[key] = new UnionContext(matchingVariants);
      } else if (matchingVariants.length === 1) {
        scopedResults.entries[key] = matchingVariants[0];
      }

      // If 0 matches, don't include
    } else if (entry.scope && validScopes.find(scope => scope.id === entry.scope.id)) {
