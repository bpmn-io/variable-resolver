...
// Initialize feel analyzer for tracking variable usage
...
function getExpressionDetails(variable, origin) {
  ...
  // Analyze the expression to extract input requirements for future prefilling
  let analysis;
  try {
  ...
  return { expression, unresolved, analysis };
}
...
/**
 * Extract input variable requirements from input mapping expressions.
 * Uses the FeelAnalyzer to determine which variables are referenced.
 *
 * @param {Array<ProcessVariable>} variables
 * @returns {Array<ProcessVariable>} input requirements (including references to existing variables)
 */
export function extractInputRequirements(variables) {
  const inputRequirements = {};
  const existingNames = new Set(variables.map(v => v.name));

  const origins = new Set();
  variables.forEach(variable => {
    variable.origin.forEach(origin => origins.add(origin));
  });

  origins.forEach(origin => {
    const expressions = getExpressions(origin);

    expressions.forEach(({ source, targetName }) => {
      try {
        const result = feelAnalyzer.analyzeExpression(source);
        if (!result.valid || !result.inputs) return;

        const { inputs } = result;

        for (const [ name, typeInfo ] of Object.entries(inputs)) {
          const key = `${name}__${origin.id}`;
          ...
          if (!inputRequirements[key]) {
            ...
            inputRequirements[key] = {
              ...
              origin: [ origin ],
              scope: origin,
              entries: typeInfoToEntries(typeInfo),
              usedBy: [ targetName ],
              provider: [],
            };
          } else {
...
/**
 * Collect all FEEL expressions from an element's input mappings and script definitions.
 *
 * @param {djs.model.Base} origin
 * @returns {Array<{ source: String, targetName: String }>}
 */
function getExpressions(origin) {
  const expressions = [];

  // Input mappings
  const ioMapping = getExtensionElementsList(origin, 'zeebe:IoMapping')[0];
  if (ioMapping?.inputParameters) {
    ioMapping.inputParameters.forEach(input => {
      if (input.source?.startsWith('=')) {
        expressions.push({ source: input.source, targetName: input.target });
      }
    });
  }
  ...
  const script = getExtensionElementsList(origin, 'zeebe:Script')[0];
  if (script?.expression?.startsWith('=') && script.resultVariable) {
    expressions.push({ source: script.expression, targetName: script.resultVariable });
  }

  return expressions;
}

function typeInfoToEntries(typeInfo) {
  if (!typeInfo || typeInfo.type !== 'context' || !typeInfo.properties) {
    return [];
  }

  return Object.entries(typeInfo.properties).map(([ name, info ]) => ({
    name,
    entries: typeInfoToEntries(info)
  }));
}
