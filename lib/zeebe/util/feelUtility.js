...
function resolveReferences(variablesToResolve, allVariables) {
  ...
  const variablesToResolveSet = new Set(
    variablesToResolve.map(({ variable }) => variable)
  );

  const variablesWithoutMappings = allVariables.filter(v =>
    !variablesToResolveSet.has(v)
  );
...
function filterForScope(context, variable) {
  ...
  const validScopeIds = new Set(variable.origin.flatMap(bo => {
    return [ bo, ...getParents(bo) ].map(e => e.id);
  }));

  for (const key in context.entries) {
    const entry = context.entries[key];

    if (validScopeIds.has(entry.scope.id)) {
