...
export default class ZeebeVariableResolver extends BaseVariableResolver {
  ...
  async getVariablesForElement(element, moddleElementOrOptions, options = {}) {
    ...
    const namesToFilter = new Set(getElementNamesToRemove(moddleElement, inputOutput));
    ...
    return variables.filter(v => {
      ...
      if (
        ...
        (v.provider || []).some(extractor => extractor !== this._baseExtractor)
      ) {
        return true;
      }

      // Filter all pre-defined variables
      return !namesToFilter.has(v.name);
    });
  }
  ...
  _expandVariables(e, context) {
    context.variables = mapVariablesByRoot(context.variables, expandHierarchicalNames);
  }
  ...
  _resolveVariables(e, context) {
    context.variables = mapVariablesByRoot(context.variables, variables => {
      const { resolvedVariables, consumedVariables } = parseVariables(variables);

      return [ ...variables, ...resolvedVariables, ...consumedVariables ];
    });
  }
}
...
function mapVariablesByRoot(rawVariables, mapper) {
  const mappedVariables = {};

  for (const key in rawVariables) {
    mappedVariables[key] = mapper(rawVariables[key]);
  }
