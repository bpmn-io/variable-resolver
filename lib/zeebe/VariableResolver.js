...
export default class ZeebeVariableResolver extends BaseVariableResolver {
  ...
  async getVariablesForElement(element, moddleElement) {
    ...
    // TODO: better code, clean up

    // same name, different scope. give priority to parent process
    const groupedByName = variables.reduce((acc, variable) => {
      acc[variable.name] = (acc[variable.name] || 0) + 1;
      return acc;
    }, {});

    // Filter variables with the same name
    const sameNameCurrentScopeVariables = variables.filter(variable => groupedByName[variable.name] > 1 && variable.scope === bo);
    ...
    return variables.filter(v => {
      ...
      if (v === sameNameCurrentScopeVariables[0]) { // TODO: check it for all
        return false;
      }
