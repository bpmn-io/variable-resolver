...
function resolveReferences(variablesToResolve, allVariables) {
  ...
  const variablesWithoutDependencies = allVariables.filter(v =>
  ...
  const rootContext = {
    name: 'OuterContext',
    entries: toOptimizedFormat(variablesWithoutDependencies)
  };
  ...
  sortedVariables.forEach(({ variable }) => {
    ...
    resolvedVariables[variable.name] = computedResult;
  });

  const newVariables = toUnifiedFormat(resolvedVariables);

  // Ensure meta-data (scope, origin) is kept from original variable
  const result = newVariables.map(newVariable => {
