...
export class BaseVariableResolver {
  ...
  async getVariablesForElement(element) {
    ...
    // (3) overwrite scopeVariables to parentScopeVariables if same name except resultVariable
    // const commonVariables = parentsScopeVariables
    //   .filter(parentScopeVar => scopeVariables.some(scopeVar => scopeVar.name === parentScopeVar.name));

    // if (commonVariables) {

    //   // separate io mappings and resultVariable
    //   if (true) { // it's a resultVariable
    //     scopeVariables = scopeVariables.filter(x => !commonVariables.some(y => y.name === x.name));
    //   } else {
    //     //parentsScopeVariables = parentsScopeVariables.filter(x => x != commonVariables[0]);
    //   }
