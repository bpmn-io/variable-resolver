...
export class BaseVariableResolver {
  ...
  _parseVariables(rawVariables) {
    ...
    for (const key in rawVariables) {
      ...
      const scopelessVariables = [];

      variables.forEach(variable => {
        const isMergeable = !!(variable.scope || !variable.usedBy);

        if (isMergeable) {
          const mergeKey = `${ variable.name }\0${ variable.scope?.id || '' }`;
          ...
          scopelessVariables.push(variable);
        }
      });

      const mergedVariables = Array.from(mergedMap.values()).concat(scopelessVariables);
...
/**
 * @param {ProcessVariable} a
 * @param {ProcessVariable} b
 */
function mergeVariables(a, b) {

  // FEEL-enriched variables may carry shape-union variants (without origin);
  // those must not be mistaken for per-origin variants
  if (!a.variants || !isPerOriginVariants(a.variants)) {
    a.variants = a.origin && a.origin.length > 1
      ? a.origin.map(o => snapshotVariant({ ...a, origin: [ o ] }))
      : [ snapshotVariant(a) ];
  }

  const bSnapshot = snapshotVariant(b);

  if (bSnapshot.origin?.length) {
    const existing = findVariantByOrigin(a.variants, bSnapshot.origin);

    if (existing) {
      enrichVariant(existing, bSnapshot);
    } else {
      a.variants.push(bSnapshot);
    }
  }
...
function snapshotVariant(variable) {
  return {
    origin: variable.origin ? [ ...variable.origin ] : variable.origin,
    type: variable.type,
    info: variable.info,
    detail: variable.detail,
    entries: variable.entries ? variable.entries.map(cloneVariable) : variable.entries,
    scope: variable.scope,
    isList: variable.isList
  };
}

function isPerOriginVariants(variants) {
  return variants.every(v => v.origin && v.origin.length);
}

function findVariantByOrigin(variants, origin) {
  return variants.find(v =>
    v.origin &&
    v.origin.length === origin.length &&
    origin.every(o => v.origin.includes(o))
  );
}

function enrichVariant(target, source) {
  if (!target.type && source.type) target.type = source.type;
  if (!target.info && source.info) target.info = source.info;
  if (!target.detail && source.detail) target.detail = source.detail;
  if (!target.entries && source.entries) target.entries = source.entries;
  if (target.isList === undefined && source.isList !== undefined) target.isList = source.isList;
}
