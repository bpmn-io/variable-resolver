...
export class BaseVariableResolver {
  ...
  _parseVariables(rawVariables) {
    ...
    for (const key in rawVariables) {
      ...
      variables.forEach(variable => {
        const mapKey = variable.name + '\0' + (variable.scope ? variable.scope.id : '');
        const existingVariable = mergedMap.get(mapKey);
        ...
        if (existingVariable) {
          ...
          mergedMap.set(mapKey, variable);
        }
      });

      const mergedVariables = Array.from(mergedMap.values());
    ...
    elements.forEach(function(element) {
      self.providers.forEach(function(provider) {
        ...
        if (result && typeof result.then === 'function') {
          pendingTasks.push(result.then(function(newVariables) {
            addVariables(self, processVariables, newVariables, element, containerElement, provider);
          }));
        } else {
          addVariables(self, processVariables, result, element, containerElement, provider);
        }
      });
    });
  ...
  async getVariablesForElement(element) {
    ...
    // collect scope and parent variables in a single pass
    var parentIds = new Set(getParents(bo).map(function(parent) {
      return parent.id;
    }));
    ...
    for (var i = 0; i < allVariables.length; i++) {
      ...
      var scopeId = variable.scope.id;
      if (scopeId === boId) {
        scopeVariables.push(variable);
      } else if (parentIds.has(scopeId)) {
...
function addVariables(resolver, processVariables, newVariables, element, containerElement, provider) {
  if (!newVariables) {
    return;
  }
  ...
  newVariables.forEach(function(variable) {
    processVariables.push({
      ...
      origin: [ element ],
      scope: variable.scope || resolver._getScope(element, containerElement, variable.name, true),
      provider: [ provider ]
    });
  });
}